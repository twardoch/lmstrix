...
class TestLMStudioClient:
    ...
    @pytest.mark.asyncio
    async def test_acompletion_success(self, mock_llm, mock_completion_response):
    ...
    @pytest.mark.asyncio
    async def test_acompletion_failure(self, mock_llm):
    ...
    @pytest.mark.asyncio
    async def test_acompletion_with_defaults(self, mock_llm, mock_completion_response):
        """Test async completion with default parameters."""
        mock_llm.complete = AsyncMock(return_value=mock_completion_response)

        client = LMStudioClient()
        result = await client.acompletion(mock_llm, "Hello")
