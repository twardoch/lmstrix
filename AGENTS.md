...
- `tiktoken`: Token counting (cl100k_base approximation)
...
<!-- lore:019cc405-28df-71b3-b5cc-d39ea0d36834 -->
* **HY-MT1.5 Jinja template: JSON-like string to Chinese translation prompt converter**: The \`translategemma.jinja2\` template accepts: (1) plain string passthrough, (2) structured dict/mapping with input/output/text or source\_lang\_code/target\_lang\_code/type fields, (3) string starting with \`translate:\` parsed via \`render\_translate\_record()\` macro using semicolon/newline-delimited key:value records (translate/input/output/vocab/context/text). The macro uses \`namespace()\` for state and handles multi-line values. Vocab entries use \`source,target\` format rendered as \`source → target\` lines. The template includes ~670 lines of language code→name mappings. System messages are filtered out before validation (lines 673-682) since TranslateGemma only supports user/assistant roles. The \`hy-mt1.5.jinja2\` template uses JSON-like parsing with \`extract\_json\_string()\`/\`extract\_json\_object()\` macros and character-by-character parsing with \`|list\` for MiniJinja compat.
...
<!-- lore:019cc405-28de-7830-9774-8e94bbc9118b -->
* **LMStudio uses MiniJinja which cannot iterate strings directly in for loops**: LMStudio's template engine is MiniJinja (Rust), not Python Jinja2. MiniJinja does NOT allow \`for ch in some\_string\` — it throws \`Expected iterable or object type in for loop: got StringValue\`. Fix: pipe strings through \`|list\` before iteration, e.g. \`for ch in (tail\[1:]) | list\`. This also means there is no built-in \`from\_json\` filter. Any JSON-like string parsing must be done via manual string operations (split, startswith, replace) with all string iterations using \`|list\`. The \`namespace()\`, macros, \`|trim\`, \`|replace\`, \`split()\`, and \`startswith()\` all work in MiniJinja. Affected template: \`hy-mt1.5.jinja2\` in \`src/lmstrix/data/jinja2\_templates/\`.

<!-- lore:019cc4a1-ca6c-7669-a56d-7d239d4e7ce7 -->
* **TranslateGemma template rejects system messages — must filter before validation**: TranslateGemma's Jinja2 chat template (both orig and custom \`translategemma.jinja2\`) calls \`raise\_exception("Conversations must start with a user prompt.")\` if \`messages\[0]\['role'] != 'user'\`. LM Studio often prepends a system message (even empty) from UI defaults or API \`system\_prompt\` field, causing this error. Fix: add a \`namespace(filtered=\[])\` loop before validation to collect only non-system messages, then validate and iterate over the filtered list. Applied at lines 673-682 of \`translategemma.jinja2\`. The \`namespace()\` pattern works in both MiniJinja and Python Jinja2. This affects all TranslateGemma sizes (4B, 12B, 27B) since they share the same template structure.
...
<!-- lore:019cc4a1-ca6e-7b34-bb78-995afce43dfb -->
* **LMStrix translation template deployment workflow**: Translation model templates (\`translategemma.jinja2\`, \`hy-mt1.5.jinja2\`) are developed in \`src/lmstrix/data/jinja2\_templates/\` then deployed by copying to LM Studio's model directory (e.g. \`/Volumes/Falstaff4T/RomeoData2/lmstudio/models/mlx-community/\<model>/chat\_template.jinja\`). Test scripts (\`translategemma\_test.py\`, \`hy-mt1.5-test.py\`) automate: deploy template → \`lms unload -a; lms load \<model>\` → HTTP POST to \`localhost:1234/api/v1/chat\` → parse response. The \`task2.md\` file defines the current input format: \`translate:;input:en;output:pl;vocab:source,target;text:...\` with semicolon or newline separators. Both templates must handle the same input format but produce model-specific prompts.
